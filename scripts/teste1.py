...
# Caminho do dicionario através do sys módulo 
...
def main():
    ...
    # encoding (removido, pois o pandas geralmente lida com isso automaticamente)
    tabela = six.text_type(tabela).encode('utf-8')
    #translate = six.text_type(translate).encode('utf-8')
    ...
    for coluna in colunas_a_traduzir:
        tabela_traduzida[coluna] = tabela[coluna].apply(lambda x: ' '.join(traduzir_colunas(palavra, translate) for palavra in re.findall(r'\b\w+\b', x)))
