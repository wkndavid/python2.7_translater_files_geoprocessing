...
def traduzir_colunas(coluna, translate):
    return ' '.join(translate.get(palavra, palavra).upper() for palavra in re.findall(r'\b\w+\b', coluna.decode('utf-8', 'ignore')))

    # return ' '.join(translate.get(palavra, palavra) for palavra in re.findall(r'\b\w+\b', coluna.decode('utf-8', 'ignore'))).upper()
...
def main():
    ...
    for coluna in colunas_a_traduzir:
        coluna_traduzida = tabela[coluna].apply(lambda x: traduzir_colunas(x, translate).decode('utf-8', 'ignore'))
        tabela_traduzida.append(coluna_traduzida)

    print(tabela_traduzida)
