...
def traduzir_palavra(palavra, dicionario):
    return dicionario.get((palavra, palavra), 'utf-8')

def traduzir_colunas(df, colunas, traducoes):
    for coluna in colunas:
        df[coluna] = df[coluna].apply(lambda x: ' '.join(traduzir_palavra(palavra, traducoes) for palavra in re.findall(r'\b\w+\b', str(x)))).upper().encode('utf-8')
    return df


# Carregando o arquivo Excel usando openpyxl
...
# Lendo os dados da planilha em um DataFrame do Pandas
data = sheet.values
columns = next(data)[:]  # Ignorar a primeira coluna (índices)
tabela = pd.DataFrame(data, columns=columns)
...
tabela_traduzida.to_excel(caminho_saida_excel, index=False)
