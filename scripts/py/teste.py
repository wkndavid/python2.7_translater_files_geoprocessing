...
def traduzir_colunas(df, colunas, traducoes):
    for coluna in colunas:
        df[coluna] = df[coluna].apply(lambda x: ' '.join(traduzir_palavra(palavra, traducoes) for palavra in re.findall(r'\b\w+\b', str(x)))).upper()
    return df
...
data = sheet.values
columns = next(data)[1:]  # Ignorar a primeira coluna (índices)
